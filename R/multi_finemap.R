...
#' finemap_methods <- check_necessary_cols(dat=BST1, finemap_methods=finemap_methods)
check_necessary_cols <- function(dat,
  ...
  for(m in finemap_methods){
    ...
    if(!all(required_dict[[m]] %in% colnames(dat))){
      ...
      missing_cols <- required_dict[[m]][!required_dict[[m]] %in% colnames(dat)]
    ...
    if(!all(suggested_dict[[m]] %in% colnames(dat))){
      missing_cols <- suggested_dict[[m]][!suggested_dict[[m]] %in% colnames(dat)]
...
multi_finemap <- function(locus_dir,
  ...
  select_cols <- colnames(dat)[!grepl(colnames(dat),
