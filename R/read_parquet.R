...
#' dat <- read_parquet(parquet_path=parquet_path, method="pandas")
#' 
#' #### Using R (SparkR) ####
#' dat <- read_parquet(parquet_path=parquet_path, method="sparkR")
#' }
read_parquet <- function(parquet_path,
    ...
    if(method=="SparkR"){
        ...
        if(!echoconda::env_exists(conda_env = conda_env)){
            echoconda::env_from_yaml() # Installs echoR by default
            conda_env <- "echoR"
        }
        if(echoconda::env_exists(conda_env = conda_env)){
